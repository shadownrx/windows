...
import { useSettings, useWindowManager } from '../sdk/host';
...
function SdkDocsApp(_props: NexAppProps) {
  ...
  const openHello = () => {
    openWindow('hello-nex', 'hello-nex', 'Hello NEX', <span style={{ fontSize: 16 }}>🚀</span>);
  };
  ...
  if (section === 'intro') {
    body = (
      <>
        ...
        <p className="sdk-footnote">
          Guía markdown: <code>docs/SDK.md</code> · Package: <code>packages/nex-os-sdk</code> · v0.1.0
        </p>
      </>
    );
    ...
    body = (
      <>
        ...
        <Code>{`import { defineApp, type NexAppProps } from '@nex-os/sdk';
...
export default defineApp({
  ...
  subscribeRegistry,
        ...
        <ul className="sdk-list">
          <li>
            <code>defineApp</code> — lo normal: registra y devuelve
          </li>
          <li>
            <code>subscribeRegistry</code> — re-render Start/Taskbar al sumar apps
          </li>
          <li>
            <code>getCommunityLauncherItems</code> — items listos para el dock
          </li>
        </ul>
      </>
    );
    ...
    body = (
      <>
        <Code>{`import { useWindowManager, useSettings } from '../sdk/host';
...
const { accentColor, addNotification } = useSettings();`}</Code>
        ...
        <ul className="sdk-list">
          <li>
            <code>useWindowManager</code> — open / close / minimize / focus
          </li>
          <li>
            <code>useSettings</code> — tema, notificaciones, volumen
          </li>
          <li>
            También: <code>useDesktop</code>, <code>useFileSystem</code>, <code>useUI</code>
          </li>
        </ul>
      </>
    );
    ...
    body = (
      <>
        <h3 className="sdk-h3">Abrir otra app</h3>
        <Code>{`openWindow('notepad', 'notepad', 'Bloc de notas', icon);`}</Code>
        <h3 className="sdk-h3">Notificación</h3>
        <Code>{`addNotification('Mi App', 'Listo.');`}</Code>
        <h3 className="sdk-h3">Props al abrir</h3>
        <Code>{`openWindow('mi-app', 'mi-app', 'Mi App', icon, { mode: 'pro' });`}</Code>
        <div className="sdk-actions">
          <button type="button" className="sdk-btn primary" onClick={openHello}>
            Probar Hello NEX
          </button>
      ...
      <aside className="sdk-rail">
        <div className="sdk-brand">
          ...
          <div>
            ...
            <div className="sdk-brand-sub">@nex-os/sdk · 0.1.0</div>
          </div>
        </div>
